...
- **land**: the argument names a branch (`into dev`, `to dev`, `dev`). Everything in open, then squash-merge on green builds, fast-forward, delete the landed branch. The target is the named branch and only that branch; `main` is a target only when the argument says `main`.
